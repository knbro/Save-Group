...
@User.on_message(filters.group & (filters.document | filters.video | filters.audio) & ~filters.edited)
async def files_handler(bot: Client, cmd: Message):
    ...
    if cmd.from_user.is_bot:
        text = "This File will be deleted in 10 minutes.\n\n" \
               ...
               f"[👉 Get File Now 👈](https://t.me/{(await Bot.get_me()).username}?start=muzik_{str(forward.message_id)})"
    else:
        text = f"{cmd.from_user.mention} Unkil,\n" \
               ...
               f"**File Name:** `{media.file_name}`\n\n" \
               f"[👉 Get Your File Now 👈](https://t.me/{(await Bot.get_me()).username}?start=muzik_{str(forward.message_id)})"
    await sendMessage(
        bot=bot,
